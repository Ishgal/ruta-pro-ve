...
import { loginAction, linkedInOAuthAction, ActionState } from '@/app/actions/auth.actions'
...
export default function LoginForm({ errorParam }: { errorParam?: string }) {
  ...
  return (
    <div className="animate-fade-in-up w-full max-w-md mx-auto">
      <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-1">Iniciar Sesión</h1>
      <p className="text-gray-500 text-sm mb-7">Bienvenido de vuelta. Ingresa tus datos.</p>
      ...
      <form action={action} className="space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <label htmlFor="password" className="hidden md:block text-sm font-semibold text-gray-700">
              Contraseña
            </label>
            <label htmlFor="password" className="md:hidden block text-xs font-bold uppercase tracking-wider text-[#1B4F8C]">
              Contraseña
            </label>
            <Link
      ...
      <p className="md:hidden text-center text-sm text-gray-500 mt-6">
        ¿No tienes cuenta?{' '}
        <Link href="/register" className="text-[#1B4F8C] font-semibold hover:underline">
          Crear Cuenta
        </Link>
      </p>
    </div>
